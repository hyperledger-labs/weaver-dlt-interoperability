...
function eciesDecryptMessage(recipientPrivateKey, cipherText, options) {
    ...
    var Rb_len = Math.floor((level + 7) / 8) * 2 + 1; // 'rLen'
    var D_len = level >> 3; // 'hLen'
    var ct_len = cipherText.length; // 'len(ct)'
    if (ct_len <= Rb_len + D_len) {
        throw new Error("Illegal cipherText length: ".concat(ct_len, " must be > ").concat(Rb_len + D_len));
    }
    ...
    var ecdsa = new elliptic.ec("p".concat(level));
    // convert bytes to usable key object
    var ephPubKey = ecdsa.keyFromPublic(Buffer.from(Rb, "hex"), "hex"); // 'R'
    var privKey = ecdsa.keyFromPrivate(recipientPrivateKey.prvKeyHex, "hex");
    var Z = privKey.derive(ephPubKey.pub); // 'z'
    var kdfOutput = hkdf(Z.toArray(), ECIESKDFOutput, null, null, options); // The 'null's correspond to 's1' and 's2', which are both
    // empty in our SNAMCC and ESCC plugin implementations
    var kbuf = Buffer.from(kdfOutput);
    var aesKey = kdfOutput.slice(0, AESKeyLength); // 'Ke'
    var hmacKey = kdfOutput.slice(AESKeyLength, AESKeyLength + HMACKeyLength); // 'Km'
    var hmacKeyHash = new options.hashFunctionKeyDerivation();
    hmacKeyHash.update(bytesToBits(hmacKey));
    var hKm = bitsToBytes(hmacKeyHash.finalize());
    var recoveredD = hmac(hKm, EM, options);
    if (D.compare(Buffer.from(recoveredD)) !== 0) {
        throw new Error("HMAC verify failed");
    }
    var iv = EM.slice(0, IVLength);
    var cipher = crypto.createDecipheriv("aes-128-ctr", Buffer.from(aesKey), iv); // The Golang package implements AES-128-CTR
...
function eciesEncryptMessage(recipientPublicKey, msg, options) {
    ...
    processOption(options);
    var ecdsa = new elliptic.ec("p".concat(level));
    var pubKey = ecdsa.keyFromPublic(recipientPublicKey.pubKeyHex, "hex");
    // Generate ephemeral key-pair
    var ephKeyPair = jsrsa.KEYUTIL.generateKeypair("EC", options.curveName);
    ...
    var Z = ephPrivKey.derive(pubKey.pub);
    var kdfOutput = hkdf(Z.toArray(), ECIESKDFOutput, null, null, options);
    ...
    var cipher = crypto.createCipheriv("aes-128-ctr", Buffer.from(aesKey), iv);
    var encryptedBytes = cipher.update(msg);
    var EM = Buffer.concat([iv, encryptedBytes]);
    var D = hmac(hKm, EM, options);
...
function checkSecurityLevel(securityLevel) {
    if (securityLevel !== 256 && securityLevel !== 384)
        throw new Error("Illegal level: ".concat(this.securityLevel, " - must be either 256 or 384"));
}
function checkHashFunction(hashAlgorithm) {
    if (!isString(hashAlgorithm))
        throw new Error("Illegal Hash function family: ".concat(hashAlgorithm, " - must be either SHA2 or SHA3"));
    var hashAlgorithmUpper = hashAlgorithm.toUpperCase();
...
function processOption(options) {
    ...
    options.suite = "".concat(options.hashAlgorithm.toLowerCase(), "-").concat(options.securityLevel);
    ...
    switch (options.suite) {
        ...
        case "sha2-256":
            ...
            break;
        default:
            throw new Error("Suite ".concat(options.suite, " not found."));
    }
    switch (options.securityLevel) {
        ...
        case 384:
            ...
            break;
        default:
            throw new Error("Security level ".concat(options.securityLevel, " not found."));
    }
    return options;
}
function hkdf(ikm, keyBitLength, salt, info, options) {
    if (!salt)
        salt = createZeroBuffer(options.hashOutputSize);
    if (!info)
        info = "";
    var key = hkdf2(bytesToBits(Buffer.from(ikm)), keyBitLength, bytesToBits(salt), info, options.hashFunctionKeyDerivation);
...
function hkdf2(ikm, keyBitLength, salt, info, Hash) {
    ...
    hmac.update(ikm);
    var key = hmac.digest();
    var hashLen = sjcl.bitArray.bitLength(key);
    var loops = Math.ceil(keyBitLength / hashLen);
    if (loops > 255) {
...
function hmac(key, bytes, options) {
    var hmac = new sjcl.misc.hmac(bytesToBits(key), options.hashFunctionKeyDerivation);
