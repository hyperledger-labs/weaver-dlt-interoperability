...
impl Network for NetworkService {
    ...
    async fn unsubscribe_event(&self, request: Request<NetworkEventUnsubscription>) -> Result<Response<Ack>, Status> {
        ...
        let delete_pub_spec_status = delete_event_pub_spec(request_id.to_string(), requested_unsub_pub_spec, conf.get_str("db_path").unwrap().to_string(), conf.get_int("db_open_max_retries").unwrap_or(10) as u32);
