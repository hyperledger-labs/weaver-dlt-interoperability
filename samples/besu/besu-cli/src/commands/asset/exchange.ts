...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
	...
	const interopContract1 = await getContractInstance(provider1, network1Config.interopContract)
	const tokenContract1 = await getContractInstance(provider1, network1Config.tokenContract)
	...
	const interopContract2 = await getContractInstance(provider2, network2Config.interopContract)
	const tokenContract2 = await getContractInstance(provider2, network2Config.tokenContract)
	...
	await tokenContract1.approve(interopContract1.address, amount, {from: sender1})
	...
	const lockContractId1 = lockTx1.logs[0].args.contractId
	...
	// Locking the asset (works only for ERC20 at this point)
	await tokenContract2.approve(interopContract2.address, amount, {from: sender2})
	...
	const lockContractId2 = lockTx2.logs[0].args.contractId
	...
	await interopContract1.claimFungibleAsset(lockContractId1, preimage, {from: recipient1})
	...
	await interopContract2.claimFungibleAsset(lockContractId2, preimage, {from: recipient2})
